...
static KeyType hashPPFM2E(const Eigen::Vector4f& f, const double AngleStep, const double DistanceStep)
{
    ...
    murmurHash(key, 4*sizeof(int), 42, &hashKey);//用公认的murmurHash函数计算哈希值
    return hashKey;
}
...
double  ppf3d_public::computeAlphaM2E(const Eigen::Vector3f& pt1, const Eigen::Vector3f& nor1, const Eigen::Vector3f& pt2)
{
    ...
    Eigen::Matrix3f R;
    computetransformRTM2E(pt1,nor1, R, Tmg);//将点云对齐到x轴的变换矩阵
    mpt = Tmg + R*pt2;
...
void    ppf3d_public::samplePCM2E(std::vector<Eigen::Vector3f> pc, std::vector<Eigen::Vector3f> nr,
	std::vector<Eigen::Vector3f> &samoled_pc, std::vector<Eigen::Vector3f> &samoled_nr, float sampleStep, int weightByCenter)
{
    std::vector< std::vector<int> > map;

    int numSamplesDim = (int)(1.0/sampleStep);//总共多少个区间
    ...
    for (int i=0; i<pc.size(); i++)
    {
        ...
        const int index = xCell*numSamplesDim*numSamplesDim+yCell*numSamplesDim+zCell;//设置cell的index

        /*#pragma omp critical
        {*/
        map[index].push_back(i);//记录每个cell中的点的index
        //  }
    }
    ...
    for (unsigned int i=0; i<map.size(); i++)//遍历每个cell
    {
        double px=0, py=0, pz=0;//质心点/加权质心点
        double nx=0, ny=0, nz=0;//质心法向量/加权质心法向量

        std::vector<int> curCell = map[i];//获取每个cell的点
...
void    ppf3d_public::computePPFFeaturesM2E(const Eigen::Vector3f& pt1, const Eigen::Vector3f& nor1,
	const Eigen::Vector3f& pt2, const Eigen::Vector3f& nor2, Eigen::Vector4f& f)
{
...
void    ppf3d_public::trainM2E(std::vector<Eigen::Vector3f>& M_pc, std::vector<Eigen::Vector3f>& M_nr)
{
    ...
    float diameter = sqrt ( dx * dx + dy * dy + dz * dz );//外接球直径
	...
	//体素化，取各个cell的质心
    samplePCM2E(M_pc, M_nr, sample_pc, sample_nr, (float)sampling_step_relativeM2E,1);
	//计算点对size，一个点对应 n 个点，那个点对应 n**2
    ...
    for (int i=0; i<numRefPoints; i++)
    {
        ...
        for (int j=0; j<numRefPoints; j++)
        {
            ...
            if (i!=j)
            {
                ...
                KeyType hashValue = hashPPFM2E(f, angle_step_radiansM2E, distanceStep);//哈希值

                double alpha = computeAlphaM2E(p1, n1, p2);
                unsigned int ppfInd = i*numRefPoints+j;//哈希表的行索引数，一行一个点对
...
void    ppf3d_public::matchM2E(std::vector<Eigen::Vector3f>& S_pc, std::vector<Eigen::Vector3f>& S_nr,
	std::vector<Pose_3DM2E>& results, const double relativeSceneSampleStep, const double relativeSceneDistance)
{
    scene_sample_stepM2E = (int)(1.0/relativeSceneSampleStep);//选取场景点的阈值

    int numAngles = (int) (floor (2 * M_PI / angle_stepM2E));
    float distanceStep = (float)distance_stepM2E;
    unsigned int n = num_ref_pointsM2E;
    ...
    // relativeSceneDistance = 0.04，越小采样的数越多
    std::vector<Eigen::Vector3f> sampled_pc, sampled_nr;
    samplePCM2E(S_pc, S_nr, sampled_pc, sampled_nr, (float)relativeSceneDistance,1);
...
#pragma omp parallel for
    ...
    for (int i = 0; i < sampled_pc.size(); i += sceneSamplingStep)
    {
        ...
        {
            if (i!=j && (sampled_pc[i]-sampled_pc[j]).norm() <= model_diameter)
            {
                ...
                while (node)
                {
                    ...
                    double alpha = alpha_model - alpha_scene;  //计算场景的匹配点对需要绕x轴转多少角度才能与模型重合
                  /*
                  ...
                  */
                    ...
                    //printf("%f\n", alpha);
                    int alpha_index = (int)(numAngles*(alpha + 2*M_PI) / (4*M_PI));//角度区间的索引数

                    unsigned int accIndex = corrI * numAngles + alpha_index;//累计表的行数+列数，即哪个模型点对应哪个角度区间投了票
        ...
        for (unsigned int k = 0; k < n; k++)//遍历每个模型点
        {
            for (int j = 0; j < numAngles; j++)//遍历alpha角度区间数
            {
                const unsigned int accInd = k*numAngles + j;//索引值
                const unsigned int accVal = accumulator[ accInd ];//票数
                if (accVal > maxVotes)
                {
                    maxVotes = accVal;
                    refIndMax = k;//哪个点对
        ...
        Eigen::Matrix4f TsgInv;//局部坐标到场景坐标
...
#pragma omp critical
		...
		if (i % 10 == 0)
		{
			std::cout << "trained: " << std::floor(100 * i / sampled_pc.size()) << "%" << std::endl;
		}
